...
**Response**: Has two large peaks and is left skewed.
...
**Response**: No, because the data is not a normal distribution.
...
``` r
set.seed(000224)
boot_educ_rerun <- gss_educ %>% 
  rerun(
```

    ## Error: <text>:4:0: unexpected end of input
    ## 2: boot_educ_rerun <- gss_educ %>% 
    ## 3:   rerun(
    ##   ^
